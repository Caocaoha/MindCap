  ...
  return (
    <motion.div 
      ...
      className={clsx(
        ...
        entry.isBookmarked 
          ? "bg-white border-yellow-200 shadow-[0_0_15px_rgba(250,204,21,0.15)] ring-1 ring-yellow-100 z-10" 
          : "bg-slate-50/50 border-slate-100 grayscale-[0.3]"
      )}
    ...
    >
      <div className="flex justify-between items-start gap-3">
        ...
        <button onClick={toggleBookmark} className={clsx("p-2 rounded-full transition-all", entry.isBookmarked ? "text-yellow-500 bg-yellow-50" : "text-slate-300 hover:text-slate-500")}>
        ...
        {showReasonInput && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="mt-3 pt-3 border-t border-slate-100">
              ...
              <textarea value={reason} onChange={e => setReason(e.target.value)} className="w-full text-xs p-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-yellow-200 outline-none resize-none" placeholder="Ghi lại bài học hoặc cảm xúc..." rows={2} autoFocus />
  ...
  return (
    <div className="p-5 pb-32 bg-slate-50/50 min-h-screen space-y-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setViewMode('diary')} className={clsx("flex items-center gap-2 px-4 py-2.5 rounded-2xl transition-all border", viewMode === 'diary' ? "bg-white border-yellow-200 text-yellow-700 shadow-sm" : "bg-slate-100 border-transparent text-slate-400")}>
      ...
      {viewMode === 'diary' && (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
          <div className="relative">
            ...
            <input type="text" placeholder="Tìm trong dòng chảy ký ức..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-white border border-slate-100 rounded-xl py-3 pl-10 pr-4 text-sm outline-none focus:ring-2 focus:ring-blue-50 placeholder:text-slate-300 shadow-sm"/>
          </div>
          <div className="flex items-center justify-between px-2">
            <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Dòng thời gian</h3>
            <div className="flex items-center gap-1 text-[9px] font-bold text-slate-300 bg-slate-100 px-2 py-1 rounded-full"><RefreshCw size={10}/> Entropy Mode: ON</div>
          </div>
          <div className="space-y-1">
            {diaryEntries.length === 0 ? (
              <div className="text-center py-20 opacity-40"><Calendar size={40} className="mx-auto mb-2 text-slate-300"/><p className="text-sm font-medium text-slate-500">Chưa có ký ức nào</p></div>
            ) : (diaryEntries.map(entry => (<DiaryItem key={entry.id} entry={entry} searchTerm={searchTerm} />)))}
          </div>
          <p className="text-[9px] text-center text-slate-300 italic px-10">"Những ký ức không được chăm sóc (bookmark) sẽ tan biến sau 40 ngày."</p>
        </motion.div>
      )}
    </div>
  );
