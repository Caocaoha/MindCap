...
import { CheckCircle2, ArrowDownCircle, Target, Zap } from 'lucide-react';
...
const FocusItem = ({ task, onComplete, onUnfocus }: { task: any, onComplete: any, onUnfocus: any }) => {
    ...
    const saveToDb = setTimeout(() => {
      if (localProgress !== task.progress) {
        ...
        if (localProgress >= task.quantity) {
          ...
          onComplete(task.id, task); // <-- Truyền cả Task object để check Frequency
        }
      }
    }, 500);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="bg-white p-0 rounded-2xl border border-blue-100 shadow-[0_4px_12px_rgba(37,99,235,0.08)] relative z-10 overflow-hidden select-none touch-none">
      {task.quantity > 1 && (
...
export const FocusZone = () => {
  const focusTasks = useLiveQuery(() => db.entries.where('type').equals('task').filter(item => item.status === 'active' && item.isFocus === true).reverse().toArray());

  // --- LOGIC HOÀN THÀNH & TÍNH ĐIỂM ---
  const completeTask = async (id: number, task: any) => {
    await db.entries.update(id, { status: 'completed', completedAt: new Date(), progress: 100 });
    
    // Nếu có tần suất lặp lại (Daily/Weekly) -> Tính là Habit
    if (task.frequency && task.frequency !== 'once') {
       await addXp('habit_log');
    } else {
       // Ngược lại -> Tính là Todo Done
       await addXp('todo_done');
    }
  };
  ...
  if (!focusTasks || focusTasks.length === 0) {
    return (
      <div className="w-full py-4 px-4 bg-white/50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center gap-2 mt-2">
        ...
        <p className="text-[10px] text-slate-400 font-medium">Chưa có Tiêu điểm hôm nay</p>
        <p className="text-[9px] text-slate-300">Vào Kho việc để đẩy task lên đây</p>
      </div>
    );
  }
