...
// --- TASK CARD (Updated Props) ---
// Thêm prop onEdit
const TaskItem = ({ task, onMoveFocus, onDelete, onEdit }: { task: Entry, onMoveFocus: (t: Entry) => void, onDelete: (id: number) => void, onEdit: (t: Entry) => void }) => {
  ...
  const handleDragEnd = (_: any, info: any) => {
    if (info.offset.x > 100) onMoveFocus(task);
  };
      ...
      >
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
             <button onClick={() => onEdit(task)} className="text-slate-300 hover:text-blue-500"><Edit3 size={14}/></button>
...
export const NewSaBan = () => {
  ...
  const [editingTask, setEditingTask] = useState<Entry | null>(null); // State sửa việc
  ...
  const focusCount = allEntries.filter(item => item.isFocus === true && item.status === 'active').length;
  ...
  const handleMoveToFocus = async (task: Entry) => {
    if (focusCount >= 4) { alert("⚠️ Tiêu điểm đã đầy (4/4)!"); return; }
    await db.entries.update(task.id!, { isFocus: true });
    if (navigator.vibrate) navigator.vibrate(50);
  };

  const handleDelete = async (id: number) => { if (confirm("Xóa vĩnh viễn?")) await db.entries.delete(id); };
  
  // Handle Save after Edit
  const handleEditSave = () => { setEditingTask(null); };
  ...
  return (
    <div className="w-full pb-32 bg-slate-50/30 min-h-screen flex flex-col">
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md px-5 py-3 border-b border-slate-100">
        <div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="Tìm kiếm..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-slate-100/50 border-none rounded-xl py-2.5 pl-9 pr-4 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-100 transition-all"/></div>
      </div>
      <div className="px-5 pt-4 space-y-8">
        {groups.doNow.length > 0 && (<section><div className="flex items-center gap-2 mb-3 text-orange-600"><Flame size={18} fill="currentColor" className="opacity-20"/><h3 className="text-xs font-black uppercase tracking-widest">Làm Ngay ({groups.doNow.length})</h3></div>{groups.doNow.map(task => (<TaskItem key={task.id} task={task} onMoveFocus={handleMoveToFocus} onDelete={handleDelete} onEdit={setEditingTask} />))}</section>)}
        {groups.plan.length > 0 && (<section><div className="flex items-center gap-2 mb-3 text-blue-600"><Star size={18} fill="currentColor" className="opacity-20"/><h3 className="text-xs font-black uppercase tracking-widest">Suy Nghĩ Thêm ({groups.plan.length})</h3></div>{groups.plan.map(task => (<TaskItem key={task.id} task={task} onMoveFocus={handleMoveToFocus} onDelete={handleDelete} onEdit={setEditingTask} />))}</section>)}
        {groups.later.length > 0 && (<section><div className="flex items-center gap-2 mb-3 text-slate-400"><Coffee size={18} className="opacity-50"/><h3 className="text-xs font-black uppercase tracking-widest">Khác ({groups.later.length})</h3></div>{groups.later.map(task => (<TaskItem key={task.id} task={task} onMoveFocus={handleMoveToFocus} onDelete={handleDelete} onEdit={setEditingTask} />))}</section>)}
